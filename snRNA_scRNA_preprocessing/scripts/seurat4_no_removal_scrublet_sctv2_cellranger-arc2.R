...
# scrublet version for cellranger-arc2
# load required libraries
# example command: Rscript /diskmnt/Projects/Users/austins2/tools/seurat4_no_removal_scrublet_cellranger-arc2.R -i /diskmnt/Projects/PDX_scRNA_analysis/matching_snRNAseq/PDAC/cellranger-arc2/HT224P1-S1Fc2A2N1Bma1_1/outs -s HT224P1-S1Fc2A2N1Bma1_1 -o HT224P1-S1Fc2A2N1Bma1_1 --scrublet /diskmnt/Projects/PDX_scRNA_analysis/matching_snRNAseq/PDAC/scrublet/HT224P1-S1Fc2A2N1Bma1_1 --detailed TRUE 
...
option_list = list(
  make_option(c("-i", "--input"),
              type="character",
              default=NULL,
              help="path to data folder (e.g. cellranger out's folder)",
              metavar="character"),
...
h5.path <- paste0(h5_dir,"/raw_feature_bc_matrix.h5")

# read in matrix
counts <- Read10X_h5(filename = h5.path)
rna <- counts$'Gene Expression' #this is the mat
atac <- counts$peaks
...
obj = CreateSeuratObject(counts = rna[,bg_id],project=opt$sample_id,min.cells = 0)


#####read in scrublet output matrix#####
scrublet.path <- paste0(opt$scrublet,"/",sample_id,"_combo_scrublet_output_table.csv", sep="")
...
obj[["percent.rb"]] <- PercentageFeatureSet(obj, pattern = "^RBS|RPL")
...
num_predicted_singlet_by_scrublet <- table(obj$predicted_doublet_rna)[[1]]
...
obj <- SCTransform(obj, vars.to.regress = c("nCount_RNA","percent.mito"),return.only.var.genes = F,vst.flavor="v2")
...
Idents(obj) = "predicted_doublet"
DimPlot(object = obj, reduction = "umap")
dev.off()

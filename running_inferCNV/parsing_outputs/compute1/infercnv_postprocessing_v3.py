...
# argv[3] = sample_name
# argv[4] = /path/to/output/folder

# make a gene to band dictionary
karyo_map = {};
#karyo_file = open("/Users/rjayasin/Desktop/HTAN_COLO/InferCNV/Karyotype_genes_hg38.txt");
#karyo_file = open("/diskmnt/Projects/Users/austins2/tools/inferCNV/genes_karyotype_hg38_gencodev32.tsv")
karyo_file = open("/storage1/fs1/dinglab/Active/Projects/austins2/tools/inferCNV/genes_karyotype_hg38_gencodev32.tsv")
karyo_file.readline();
...
output.to_csv(sys.argv[4]+"/"+sys.argv[3]+"_arm_level.tsv",sep = "\t",index=False);
output2 = pd.DataFrame().from_dict(cell_cnv_dict_gene_level,orient="index").reset_index();
output2.to_csv(sys.argv[4]+"/"+sys.argv[3]+"_gene_level.tsv",sep = "\t",index=False);
output2_transposed = output2.transpose(copy=True)
output2_transposed.columns = output2_transposed.iloc[0]
output2_transposed = output2_transposed.drop(output2_transposed.index[0])
print(output2_transposed)
output2_transposed.to_csv(sys.argv[4]+"/"+sys.argv[3]+"_gene_level_T.tsv",sep = "\t" ,index=True, header=True);

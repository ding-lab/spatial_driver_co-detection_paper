...
# -c /diskmnt/Projects/HTAN_analysis_2/PDAC/xenium/snvs_project/cell_types/compiled_cell_types/SSP001P1-Fp1U1_cell_types_v7.tsv \
...
option_list = list(
    ...
    make_option(c("-t", "--tumor_cell_label"),
                ...
                help="comma separated list of neoplastic cell labels in the object. All cells with these labels will be included in the 'Cancer_cell' label in the contingency table.",
                metavar="character"),
...
for (assay in assays_with_snv_probes) {
    ...
    statistics_table_results <- data.frame(Variant.probe.name = rep(NA, number_of_variants),
                                           ...
                                           proportion.test.value.1..Proportion.of.cancer.cells.with.at.least.one.variant.allele.out.of.total.number.of.cancer.cells.with.variant.site.probe = rep(NA, number_of_variants), # Proportion.test.value.1 # true positive rate
                                           proportion.test.value.2..Proportion.of.normal.cells.with.at.least.one.variant.allele.out.of.total.number.of.normal.cells.with.variant.site.probe = rep(NA, number_of_variants), # Proportion.test.value.2
                                           Proportion.of.cancer.cells.with.only.reference.allele.out.of.total.number.cancer.cells.with.variant.site.probe = rep(NA, number_of_variants),
                                           Proportion.of.normal.cells.with.only.reference.allele.out.of.total.number.normal.cells.with.variant.site.probe = rep(NA, number_of_variants),
                                           # false.positive.rate.across.all.normal.cells.in.sample..sample.specific = rep(NA, number_of_variants), # don't do this at the individual sample level as it is mis-representative. not every cancer population has every mutation
                                           # false.negative.rate.across.all.cancer.cells.in.sample..sample.specific = rep(NA, number_of_variants), # don't do this at the individual sample level as it is mis-representative. not every cancer population has every mutation
    ...
    for (i in 1:length(variant_alleles[,"V1"])) {
        ...
        data["Cancer_cell","ALT"] <- length(obj@meta.data[ (obj@meta.data$tumor_cell_call == "Cancer_cell") & (obj@meta.data[, paste0(variant_key,"_",assay,"_call")] == "ALT") & (!(is.na(obj@meta.data$tumor_cell_call))) & (!(is.na(obj@meta.data[, paste0(variant_key,"_",assay,"_call")]))), "seurat_clusters" ])
        data["Cancer_cell","WT"] <- length(obj@meta.data[ (obj@meta.data$tumor_cell_call == "Cancer_cell") & (obj@meta.data[, paste0(variant_key,"_",assay,"_call")] == "WT" ) & (!(is.na(obj@meta.data$tumor_cell_call))) & (!(is.na(obj@meta.data[, paste0(variant_key,"_",assay,"_call")]))) , "seurat_clusters" ])
        data["Normal_cell","ALT"] <- length(obj@meta.data[ (obj@meta.data$tumor_cell_call == "Normal_cell") & (obj@meta.data[, paste0(variant_key,"_",assay,"_call")] == "ALT" ) & (!(is.na(obj@meta.data$tumor_cell_call))) & (!(is.na(obj@meta.data[, paste0(variant_key,"_",assay,"_call")]))) , "seurat_clusters" ])
        data["Normal_cell","WT"] <- length(obj@meta.data[ (obj@meta.data$tumor_cell_call == "Normal_cell") & (obj@meta.data[, paste0(variant_key,"_",assay,"_call")] == "WT" ) & (!(is.na(obj@meta.data$tumor_cell_call))) & (!(is.na(obj@meta.data[, paste0(variant_key,"_",assay,"_call")]))) , "seurat_clusters" ])
        ...
        if ((sum(counts_df[ ,variant_key]) > 0) & (sum(counts_df[ ,wt_key]) >0)) {
            if (("Normal_cell" %in% rownames(data)) & ("Cancer_cell" %in% rownames(data))) {
                if ((sum(data["Normal_cell",]) !=0) & (sum(data["Cancer_cell",]) !=0)) {
                    ...
                    print(phi.effectsize) # proportion.test.value.1..Proportion.of.variant.allele.probes.out.of.total.number.of.variant.site.probes.in.cancer.cells
                    statistics_table_results[i, "proportion.test.value.1..Proportion.of.cancer.cells.with.at.least.one.variant.allele.out.of.total.number.of.cancer.cells.with.variant.site.probe"] <- variant_prop_test$estimate[1][[1]] # true.positive.rate if the sample actually has the mutation
                    statistics_table_results[i, "proportion.test.value.2..Proportion.of.normal.cells.with.at.least.one.variant.allele.out.of.total.number.of.normal.cells.with.variant.site.probe"] <- variant_prop_test$estimate[2][[1]] # false.positive.rate if the sample actually has the mutation.
                    statistics_table_results[i, "Proportion.of.cancer.cells.with.only.reference.allele.out.of.total.number.cancer.cells.with.variant.site.probe"] <- data["Cancer_cell","WT"] / (data["Cancer_cell","ALT"] + data["Cancer_cell","WT"]) #false.negative.rate if the sample actually has the mutation
                    statistics_table_results[i, "Proportion.of.normal.cells.with.only.reference.allele.out.of.total.number.normal.cells.with.variant.site.probe"] <- data["Normal_cell","WT"] / (data["Normal_cell","ALT"] + data["Normal_cell","WT"]) #true.negative.rate if the sample actually has the mutation
                    # statistics_table_results[i, "false.positive.rate.across.all.normal.cells.in.sample..sample.specific"] <- data["Normal_cell","ALT"] / statistics_table_results[i, "total.number.of.normal.cells.in.sample"] # don't do this at the individual sample level as it is mis-representative. not every cancer population has every mutation
                    # statistics_table_results[i, "false.negative.rate.across.all.cancer.cells.in.sample..sample.specific"] <- data["Cancer_cell","WT"] / statistics_table_results[i, "total.number.of.cancer.cells.in.sample"] # don't do this at the individual sample level as it is mis-representative. not every cancer population has every mutation

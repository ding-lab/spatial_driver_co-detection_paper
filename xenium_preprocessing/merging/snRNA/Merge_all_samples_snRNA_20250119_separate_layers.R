##!/usr/bin/env Rscript
##################################################################
## Merge Xenium objects with variant probes.                    ##
...
for (i in iterables) {
    ...
    cell_types <- read.table(manual_cell_type_path, header=F, sep = '\t')
    obj_new$original_object_barcode <- rownames(obj_new@meta.data)
    colnames(cell_types) <- c("barcode","cell_type")
    cell_types$barcode <- paste0(sampleID,"_",cell_types$barcode)
    ...
    # There are no snv-targeting probes in the snRNA data
...
# don't need to do the following for samples generated as sc/snRNA that don't target specific variants/genes.
# Can't just do normal merging where features present in one object but not another are auto filled in with zeros because unlike in single cell data if a gene is present in the matrix of one xenium seurat object but not another it is becase  is 
...
for (i in iterables) {
    ...
    sampleID = unique(obj_new$sample_ID)
    print(sampleID)
    obj_new$nCount_RNA_individual <- obj_new$nCount_RNA
    obj_new$nFeature_RNA_individual <- obj_new$nFeature_RNA
    ...
    #############
    # the below block is focused on filtering objects if you are subseting their gene lists. We aren't doing that here so no need to have this block   
    ...
    # objects have already been filtered when initially processed.
    # obj_new <- subset_opt(obj_new, subset = nCount_Xenium > opt$counts_cutoff)
    non_empty_cells <- Cells(obj_new)
    #############
    # there are no images in the sc/snRNA objects so no need to include this part here.
    ...
    #############
    DefaultAssay(obj_new) <- "RNA"
    obj_new <- NormalizeData(obj_new, assay = "RNA")
    obj_new <- FindVariableFeatures(obj_new, assay = "RNA")
    obj_new <- ScaleData(obj_new, assay = "RNA")
    obj_new <- SCTransform(obj_new, assay = "RNA", return.only.var.genes = F, vst.flavor="v2")
...
# If you try to use JoinLayers() on a Seurat v3 assay you get the following error.
...
# It turns out that the Xenium assay in the individual object along with the control probe assays is a Seurat3 assay instead of a Seurat5 assay so when 
...
# for the sake of consistency and my sanity I'm just changing everything to assay v5 here
# This mean I can now look at the normalized data in a slightly different fashion downstream but we will get to that later.
for (i in iterables) {
    obj_new <- seurat.list[[i]]
    assay_list <- Assays(obj_new)
    for (assay in assay_list) {
...
print("SCTransform")
obj.merge <- SCTransform(obj.merge, assay = "RNA", return.only.var.genes = F, vst.flavor="v2", conserve.memory = F)
print(SCTResults(obj.merge, slot="umi.assay"))
# $`1`
# [1] "RNA"
# $`2`
...
# $`9`
# [1] "RNA"
# if you don't do JoinLayers() on the merged object then everything gets handled separately. It literally runs SCTransform on each layer separately
...
# to run PrepSCTMarkers so we can run FindAllMarkers or FindMarkers later
# This should already evaluate to `RNA` but some people were running into issues where it didn't on github so I'm going to do it just in case.
for (i in iterables) {
    slot(object = obj.merge@assays$SCT@SCTModel.list[[i]], name="umi.assay") <- "RNA"
}
# $`1`
# [1] "RNA"
# $`2`
...
saveRDS(obj.merge, paste0(out_dir,"/",prefix,"_separate_SCT_noUMAPs.rds"))
write.table(obj.merge@meta.data, paste0(out_dir,"/",prefix,"_separate_SCT_noUMAPs_20250119_metadata.tsv"), quote=F, sep='\t')
# this generates a single reduction despite the multiple SCT models
...
print("plotting")
pdf(paste0(out_dir,"/","Elbowplot_",prefix,"_separate_SCT_merge.pdf"))
...
# for the analysis of the spatial driver co-detection paper this doesn't matter as we weren't using any snRNA umaps or updating the cell type/state labels.
for (i in 1:12) {
    dimen = dims[i]
    cluster = clusters[i]
    ...
    pdf(paste0(out_dir,"/Dimplots_PDAC_merge_primary_KRAS_20241205_separate_SCT_",cluster,"PC.pdf"), useDingbats=FALSE, height=8, width = 15)
    ...
    print(rasterize(FeaturePlot(obj.merge, reduction = paste("umap.",cluster,"PC", sep=""), features = c("nFeature_RNA"), raster = F), layers="Point", dpi=300))
    print(rasterize(FeaturePlot(obj.merge, reduction = paste("umap.",cluster,"PC", sep=""), features = c("nCount_RNA"), raster = F), layers="Point", dpi=300))
...
obj.merge <- FindNeighbors(obj.merge, reduction = "pca", dims = 1:50)#, force.recalc = T) #seurat no longer supports the force.recalc parameter in this function. It will now always recalculate the neighborhoods.
obj.merge <- FindClusters(obj.merge, resolution = 0.3)
saveRDS(obj.merge, paste0(out_dir,"/",prefix,"_separate_SCT_umaps.rds"))
write.table(obj.merge@meta.data, paste0(out_dir,"/",prefix,"_separate_SCT_20250119_metadata.tsv"), quote=F, sep='\t')
library(future)
options(future.globals.maxSize= +Inf)
...
write.table(degs,paste0(out_dir,"/",prefix,"_separate_SCT_FindAllMarkers_seurat_clusters_50PC_res0.3_20250119.tsv"),sep="\t",quote=FALSE)
plan(sequential)

# The below is for plotting image based data to check that it has been preserved appropriately after merging.
# scRNA/snRNA data has no associated image data so no need to run the below.
...
# pdf(paste(out_dir,"/",prefix,"_separate_SCT_plotting_check.pdf", sep=""), useDingbats=FALSE, height=8, width = 15)
# assay_list <- Assays(obj.merge)
# snv_assay <- assay_list[grepl("snv",assay_list)][1]
# DefaultAssay(obj.merge) <- snv_assay # this needs to be done befor you crop or the crop will not be associated with the correct assay and you can't plot variant probes
...
# dev.off()
# print("testing plotting of non-SNV related information")
# pdf(paste(out_dir,"/",prefix,"_separate_SCT_plotting_check_snvs.pdf", sep=""), useDingbats=FALSE, height=8, width = 15)

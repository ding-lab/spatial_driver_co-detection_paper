...
# adding in total cells where variant site was detected to plot
...
print("re-adding proportions to include those not previously included") #technically don't need to do this anymore after v5 but it is easier to just keep it in rather than re-write the script
all_sample_summary$prop.var.in.cancer = all_sample_summary$Number.of.cancer.cells.with.the.variant.probe / all_sample_summary$total.number.of.cancer.cells.with.either.variant.site.probe
all_sample_summary$prop.ref.in.cancer = all_sample_summary$Number.of.cancer.cells.with.the.reference.probe / all_sample_summary$total.number.of.cancer.cells.with.either.variant.site.probe
all_sample_summary$prop.var.in.non.cancer = all_sample_summary$Number.of.non.cancer.cells.with.the.variant.probe / all_sample_summary$total.number.of.normal.cells.with.either.variant.site.probe
all_sample_summary$prop.ref.in.non.cancer = all_sample_summary$Number.of.non.cancer.cells.with.the.reference.probe / all_sample_summary$total.number.of.normal.cells.with.either.variant.site.probe
...
for (i in iterables) {
    ...
    cancer_alt_sample_summary <- sample_summary[c(sample_summary$sample.ID == sample),c("Variant.probe.name","prop.var.in.cancer","sample.ID","Number.of.cancer.cells.with.the.variant.probe","total.number.of.cancer.cells.with.either.variant.site.probe")]
    colnames(cancer_alt_sample_summary) <- c("probe_site","probe_proportion","sample.ID","probe_count","cell_total")
    cancer_alt_sample_summary$cell_type <- "Cancer cells"
    cancer_alt_sample_summary$probe_target <- "Variant allele"
    cancer_wt_sample_summary <- sample_summary[c(sample_summary$sample.ID == sample),c("Variant.probe.name","prop.ref.in.cancer","sample.ID","Number.of.cancer.cells.with.the.reference.probe","total.number.of.cancer.cells.with.either.variant.site.probe")]
    colnames(cancer_wt_sample_summary) <- c("probe_site","probe_proportion","sample.ID","probe_count","cell_total")
    cancer_wt_sample_summary$cell_type <- "Cancer cells"
    cancer_wt_sample_summary$probe_target <- "Reference allele"
    normal_alt_sample_summary <- sample_summary[c(sample_summary$sample.ID == sample),c("Variant.probe.name","prop.var.in.non.cancer","sample.ID","Number.of.non.cancer.cells.with.the.variant.probe","total.number.of.normal.cells.with.either.variant.site.probe")]
    colnames(normal_alt_sample_summary) <- c("probe_site","probe_proportion","sample.ID","probe_count","cell_total")
    normal_alt_sample_summary$cell_type <- "Normal cells"
    normal_alt_sample_summary$probe_target <- "Variant allele"
    normal_wt_sample_summary <- sample_summary[c(sample_summary$sample.ID == sample),c("Variant.probe.name","prop.ref.in.non.cancer","sample.ID","Number.of.non.cancer.cells.with.the.reference.probe","total.number.of.normal.cells.with.either.variant.site.probe")]
    colnames(normal_wt_sample_summary) <- c("probe_site","probe_proportion","sample.ID","probe_count","cell_total")
    ...
    sample_complete_table$cell_total = paste0("n_cell=",sample_complete_table$cell_total)
        ...
        if (grepl("ALT",alt_probe)) {
            ...
            p1 <- ggplot(sample_probe_table_complete, aes(x = cell_type, y = probe_count, fill = probe_target)) + 
                ...
                geom_text(data = subset(sample_probe_table_complete, probe_target == "Variant allele"),
                          aes(label = cell_total, x = cell_type, y = probe_total),
                ...
                labs( y = paste0("Cell count"),
                      #x = "Treatment", 
                      title = paste0("Count of cells with binary variant probe count"),
                      fill = alt_probe) +#,
    ...
    pdf(paste0(out_dir,"/",sample,"/",sample,"_barplots_cell_based_counts_p_value_alt_v6.pdf"),useDingbats = F, height=5, width = 5)
        ...
        if (grepl("ALT",alt_probe)) {
            ...
            p1 <- ggplot(sample_probe_table_complete, aes(x = cell_type, y = probe_proportion, fill = probe_target)) +
                ...
                geom_signif(comparisons=list(c("Normal cells", "Cancer cells")), annotations=paste0("Adj. prop. test p = ", formatC(all_sample_summary[(all_sample_summary$sample.ID == sample)& (all_sample_summary$Variant.probe.name == alt_probe),"bonferroni.corrected.prop.test.pval"], format = "E", digits = 2)),
                            y_position = 1.1*df_max_round, tip_length = 0, vjust=0) +
                geom_text(data = subset(sample_probe_table_complete, probe_target == "Variant allele"),
                          aes(label = cell_total, x = cell_type, y = df_max_round),
                ...
                labs( y = paste0("Proportion"),
                      #x = "Treatment",
                      title = paste0("Proportion of cells with binary variant probe count"),
                      fill = alt_probe) +#,
                #subtitle= paste0("Proportion test adjusted p = ",all_sample_summary[(all_sample_summary$sample.ID == sample)& (all_sample_summary$Variant.probe.name == alt_probe),"bonferroni.corrected.prop.test.pval"])) +
                theme(plot.title=element_text(size=18, hjust=0.5, face="bold", colour="black", vjust=1)) +
                theme(plot.subtitle=element_text(size=12, hjust=0.5, face="italic", color="black"))
    ...
    pdf(paste0(out_dir,"/",sample,"/",sample,"_barplots_cell_based_proportions_p_value_alt_v6.pdf"),useDingbats = F, height=5, width = 5)
    for ( k in 1:length(list_of_plots)) {
        print(list_of_plots[[k]])
    }
    dev.off()

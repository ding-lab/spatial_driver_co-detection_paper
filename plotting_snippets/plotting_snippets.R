# figure 1b (this code is also present in `spatial_driver_co-detection_paper/bamrc/lab_server/plotting_summary_heatmap_v2.R`. The version generated by `plotting_summary_heatmap_v2.R` is what was used in the figure 1b).
...
# Extended data figure 3 violin plots and bar plots of detection rate vs expression level
...
dev.off()
pdf(paste0(out_dir,"/","Expression_vs_total_detection_",snRNA_prefix,"_scatterplot.pdf"),useDingbats = F,height=10, width=10) # the plots from this pdf were used to make Figure 2b and Extended Data Figure 3a
for (variant in variants_with_snRNA) {
    print(variant)
    gene <- unlist(strsplit(variant, "-"))[1]
    mutant_snRNA_samples = unlist(strsplit(variant_xenium_snRNA$snRNA_sample[variant_xenium_snRNA$Variant == variant], ","))
...
dev.off()
# writing the all of the cell types the sn/scRNA samples to a single csv file. This will be a supplementary table.
...
# Figure 2d, 2e,2h, 2i, spatial image plots for HT268B1 uses the following code:
...
# the zoom in shown in figure 2h-i is from where I crop to and create the zoom1 fov below
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_4color_fov.with.snvs_crop_darkblue.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots)))
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_4color_fov.with.snvs_crop_darkblue_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots)))
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
zoom1 <- Crop(obj[["fov"]], x = c(4700, 5200), y = c(1900, 2400), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_4color_zoom1_crop_darkblue.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_4color_zoom1_crop_darkblue_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
# Figure 2k, 2l, 2o, 2p spatial image plots for SP001P1-Fp1U1 uses the following code:
# cd /diskmnt/Projects/HTAN_analysis_2/PDAC/xenium/snvs_project/cell_types/neoplastic_normal_unknown/coord_test/
# conda activate seurat5
# Rscript plotting_neoplastic_labels_7_darkblue_ceiling1.R neo_norm_unk_table_v7.tsv /diskmnt/Projects/HTAN_analysis_2/PDAC/xenium/snvs_project/cell_types/neoplastic_normal_unknown/coord_test/darkblue_ceiling1_v7/
# the full section images of the cell segmentation colored by neoplastic vs normal label are from the output of the above script specifically the *_neoplastic_normal_unknown_v4_image_2color.pdf images and the legend comes from *_neoplastic_normal_unknown_v4_image_2color_legend.pdf. by doing it this way I am able to consistently size all of the plots no matter if they are [dimplots or featureplots] or [Imagedimplots or Imagefeatureplots] and then still get the legends. Doing it this way allows the DimPlots and Featureplots to be perfectly square.
# the zoom in shown in figure 2o-p is from where I crop to and create the zoom1.1 fov below.
# there are no normal duct or panin cells in this case as it is a metastasis
# the H&E was generated by loading the HEX-SIFT aligned image into Xenium explorer and then exporting the current view with only the H&E image layer active and the cells and transcripts unchecked.
...
zoom1.1 <- Crop(obj[["fov"]], x = c(5500, 6000), y = c(5300, 5800), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom1.1"]] <- zoom1.1
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom1'
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_4color_zoom1.1_darkblue_ceiling1.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_4color_zoom1.1_darkblue_ceiling1_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
DefaultBoundary(obj[["fov.with.snvs"]]) <- "segmentation"
p13 <- rasterize(ImageDimPlot(obj, fov = "fov.with.snvs", group.by="neoplasm_normal_unknown", cols = c('#009E73','#CC79A7','#5c5c5c'), border.color = NA, axes = TRUE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position="none") + coord_fixed(ratio=1), layers="Polygon", dpi=600)
...
# needed for figure 3, related text and Supplementary Tables 9, 10, and 11.
...
p1 <- DimPlot(obj, group.by = "TP53_copy_ratio", reduction = "umap.30PC", pt.size=0.3, label=F, raster=FALSE) + scale_x_continuous(limits = dim_xlims) + scale_y_continuous(limits = dim_ylims) + theme(legend.position="none") + coord_fixed(ratio=xy_ratio) #used the legend from here in 3c (right)
p2 <- DimPlot(obj, group.by = "TP53_copy_ratio", reduction = "umap.30PC", pt.size=0.3, label=F, raster=FALSE) + scale_x_continuous(limits = dim_xlims) + scale_y_continuous(limits = dim_ylims) + coord_fixed(ratio=xy_ratio) # used the umap from here in 3c (right)
...
print(rasterize(p1, layers='Point', dpi=300))
print(rasterize(p2, layers='Point', dpi=300))
p1 <- DimPlot(obj, group.by = "cell_type_xenium_variant_v1", reduction = "umap.30PC", pt.size=0.3, label=F, raster=FALSE) + scale_x_continuous(limits = dim_xlims) + scale_y_continuous(limits = dim_ylims) + theme(legend.position="none") + coord_fixed(ratio=xy_ratio) # used the umap from here in 3b (left)
p2 <- DimPlot(obj, group.by = "cell_type_xenium_variant_v1", reduction = "umap.30PC", pt.size=0.3, label=F, raster=FALSE) + scale_x_continuous(limits = dim_xlims) + scale_y_continuous(limits = dim_ylims) + coord_fixed(ratio=xy_ratio) # used the umap from here in 3b (left)
p1 <- p1 + theme(plot.title = element_blank(),axis.title.x = element_blank(),axis.title.y = element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank(),axis.text.y=element_blank(),axis.ticks.y=element_blank())
print(rasterize(p1, layers='Point', dpi=300))
print(rasterize(p2, layers='Point', dpi=300))
dev.off()
...
# Figure 3e was taken from the output files HT65B1-H1A1A4U1_neoplastic_normal_unknown_v4_image_2color.pdf and the legend from *_neoplastic_normal_unknown_v4_image_2color_legend.pdf generated by running the following commands:
# cd /diskmnt/Projects/HTAN_analysis_2/PDAC/xenium/snvs_project/cell_types/neoplastic_normal_unknown/coord_test/
# conda activate seurat5
# mkdir darkblue_ceiling1_v7
# Rscript plotting_neoplastic_labels_7_darkblue_ceiling1.R neo_norm_unk_table_v7.tsv /diskmnt/Projects/HTAN_analysis_2/PDAC/xenium/snvs_project/cell_types/neoplastic_normal_unknown/coord_test/darkblue_ceiling1_v7/

# For Figure 3f was taken from the output file *_barplots_count_based_counts_p_value_alt_v6.pdf generated by running the following commands:
# cd /diskmnt/Projects/HTAN_analysis_2/PDAC/xenium/snvs_project/cell_types/neoplastic_normal_unknown/coord_test/
# conda activate seurat5
# mkdir darkblue_ceiling1_v7
# Rscript plotting_barplots_alt_count_v6.R &> plotting_barplots_alt_count_v6.log

# Figure 3g and 3h analysis/plots were generated below:
...
out_dir = "/diskmnt/Projects/HTAN_analysis_2/PDAC/xenium/snvs_project/cell_types/neoplastic_normal_unknown/coord_test/darkblue_ceiling1_v7/"
zoom2 <- Crop(obj[["fov.with.snvs"]], x = c(1690, 2190), y = c(2000, 2500), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom2_crop_darkblue.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom2_crop_darkblue_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
# Extended Data Figure 3e was taken from the output file *_box_plot_proportion_alt_cells_v6_subclone.pdf generated by running the following commands:
# cd /diskmnt/Projects/HTAN_analysis_2/PDAC/xenium/snvs_project/cell_types/neoplastic_normal_unknown/coord_test/
# conda activate seurat5
# mkdir darkblue_ceiling1_v7
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v6_subclone_image_2color_zoom1_crop_darkblue.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v6_subclone_image_2color_zoom1_crop_darkblue_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
zoom2 <- Crop(obj[["fov"]], x = c(3000, 3500), y = c(4750, 5250), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom2"]] <- zoom2 # this resolution (1000 µm by 1000 µm) ended up being too low when the plots were shrunk down. 
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom2'
DefaultBoundary(obj[["zoom2"]]) <- "segmentation"
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v4_subclone_image_2color_zoom2_crop_darkblue.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v4_subclone_image_2color_zoom2_crop_darkblue_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
coordinates <- Embeddings(obj, reduction = "umap.30PC")
dim_names <- colnames(coordinates)
...
# Spatial plots for figure 4f and 4g and the bar plots for Extended Data figure 7b
...
obj$uniform_background_color = "darkblue"
# use zoom1 = clone 2
zoom1 <- Crop(obj[["fov"]], x = c(4500, 5000), y = c(5400, 5900), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom1"]] <- zoom1
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom1'
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15 
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom1_darkblue_ceiling1.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
p13 <- rasterize(ImageDimPlot(obj, fov = "zoom1", group.by="neoplasm_normal_unknown", cols = c('#009E73','#CC79A7','#5c5c5c'), border.color = NA, axes = TRUE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + coord_fixed(ratio=1), layers="Polygon", dpi=600)
p14 <- rasterize(ImageDimPlot(obj, fov = "zoom1", group.by="cell_type", border.color=NA, axes = TRUE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + coord_fixed(ratio=1), layers="Polygon", dpi=600)
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom1_darkblue_ceiling1_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_clone_normal_unknown_v3_image_2color_zoom1_darkblue_ceiling1_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()

zoom2 <- Crop(obj[["fov"]], x = c(4350, 4850), y = c(3600, 4100), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom2"]] <- zoom2
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom2'
#Simplified_cell_type = c("Normal" = "#009E73","Clone_2" = "#eee461","Clone_1" = "#CC79A7")
DefaultBoundary(obj[["zoom2"]]) <- "segmentation"
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom2_darkblue_ceiling1.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
p13 <- rasterize(ImageDimPlot(obj, fov = "zoom2", group.by="neoplasm_normal_unknown", cols = c('#009E73','#CC79A7','#5c5c5c'), border.color = NA, axes = TRUE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + coord_fixed(ratio=1), layers="Polygon", dpi=600)
p14 <- rasterize(ImageDimPlot(obj, fov = "zoom2", group.by="cell_type", border.color=NA, axes = TRUE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + coord_fixed(ratio=1), layers="Polygon", dpi=600)
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom2_darkblue_ceiling1_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_clone_normal_unknown_v3_image_2color_zoom2_darkblue_ceiling1_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_clone_normal_unknown_v3_image_2color_fov.with.snvs_darkblue_ceiling1_legend.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
zoom1.1 <- Crop(obj[["fov"]], x = c(590, 990), y = c(4140,4540), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
...
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
# this is the tumor and normal duct from case HT061P1 piece HT061P1-S1P1A1 (still for figure 5b)
...
obj@meta.data["KRAS_detected"][obj@meta.data[paste0(G12D_key,"_",assay,"_count")] > 0 & obj@meta.data[paste0(G12V_key,"_",assay,"_count")] ] <- "KRAS_p.G12D_p.G12V_variant"
obj$KRAS_detected <- factor(obj$KRAS_detected, levels = c("KRAS_p.G12_probe_not_detected","KRAS_p.G12_reference","KRAS_p.G12V_variant","KRAS_p.G12D_variant","KRAS_p.G12D_p.G12V_variant"))
zoom1.1 <- Crop(obj[["fov"]], x = c(1350,1750), y = c(2600, 3000), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom1.1"]] <- zoom1.1 # this resolution (1000 µm by 1000 µm) ended up being too low when the plots were shrunk down. 
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom1'
DefaultBoundary(obj[["zoom1.1"]]) <- "segmentation"
...
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
zoom2.1 <- Crop(obj[["fov"]], x = c(3950, 4350), y = c(2750,3150), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
...
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
# The fov.with.snvs is in supplement: Extended Data Figure 8c
DefaultBoundary(obj[["fov.with.snvs"]]) <- "segmentation"
...
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
# This part is for the stacked bar plot in Extended Data Figure 8e
i = 29 # HT061P1-S1P1A1L1U1
sample = input_table[i,"Sample_ID"]
rds_obj = input_table[i,"Xenium_snv_object"]
dir.create(paste0(out_dir,"/",sample))
...
# Extended Data Figure 8a, 8b, and 8d and figure 5c
# conda activate seurat5
# plotting the zoomed in images for figure 5 related supplement
# Plotting features for the pre-cancer progression part zoomed in on regions of normal, PanIN, and PDAC
library(Seurat)
...
zoom1.1 <- Crop(obj[["fov"]], x = c(4900, 5300), y = c(2370,2770), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom1.1"]] <- zoom1.1
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom1'
#Simplified_cell_type = c("Normal" = "#009E73","Normal duct" = "#56B4E9","PanIN" = "#eee461","PDAC" = "#CC79A7")
DefaultBoundary(obj[["zoom1.1"]]) <- "segmentation"
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom1.1_darkblue_ceiling1_fig5_supplement.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #  (Extended Data Figure 8b)
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom1.1_darkblue_ceiling1_legend_fig5_supplement.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #  (Extended Data Figure 8b)
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()

zoom2.1 <- Crop(obj[["fov"]], x = c(4000, 4400), y = c(1550,1950), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom2.1"]] <- zoom2.1 # this resolution (1000 µm by 1000 µm) ended up being too low when the plots were shrunk down. 
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom1'
DefaultBoundary(obj[["zoom2.1"]]) <- "segmentation"
# The colors are based on the order in which the factor occurs in the metadata. 
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom2.1_darkblue_ceiling1_fig5_supplement.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) # use this in supplementary figure (Extended Data Figure 8b)
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom2.1_darkblue_ceiling1_legend_fig5_supplement.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) # use this in supplementary figure (Extended Data Figure 8b)
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()

zoom3.1 <- Crop(obj[["fov"]], x = c(1800, 2200), y = c(845,1245), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom3.1_darkblue_ceiling1_fig5_supplement.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) # use this in supplementary figure (Extended Data Figure 8b)
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom3.1_darkblue_ceiling1_legend_fig5_supplement.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) # use this in supplementary figure (Extended Data Figure 8b)
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_fov.with.snvs_darkblue_ceiling1_fig5_supplement.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) # use this in supplementary figure (Extended Data Figure 8a)
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_fov.with.snvs_darkblue_ceiling1_legend_fig5_supplement.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) # use this in supplementary figure (Extended Data Figure 8a)
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
# this is for the stacked bar plot for figure 5 supplement (Extended Data Figure 8d)
library(epitools)
library(effectsize)
...
draw(combined) # this is not in the supplement right now
dev.off()
# this is figure 5d
...
# Andre and I will then use the cell barcodes there to re-label just the PanIN from that detached region so that they are excluded form the layer calculation. 
# conda activate seurat5
...
# i = 19 # SP001P1-Fp1U1 # G12D 
# i = 26 # HT270P1-S1H1A1US2_1 # G12D #decided to replace with G12V HT125P1-S1H8A1U1
# i = 27 # SP002C1-Fp1U2 # G12V
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom1.1_darkblue_ceiling1_fig6.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
p13 <- rasterize(ImageDimPlot(obj, fov = "zoom1.1", group.by="neoplasm_normal_unknown", cols = c('#009E73','#CC79A7','#5c5c5c'), border.color = NA, axes = TRUE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + coord_fixed(ratio=1), layers="Polygon", dpi=600)
p14 <- rasterize(ImageDimPlot(obj, fov = "zoom1.1", group.by="cell_type", border.color=NA, axes = TRUE) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + coord_fixed(ratio=1), layers="Polygon", dpi=600)
...
offset = length(list_of_plots)
list_of_plots[[1+offset]] <- p15
pdf(paste0(out_dir,"/",sample,"/",sample,"_neoplastic_normal_unknown_v3_image_2color_zoom1.1_darkblue_ceiling1_legend_fig6.pdf"),useDingbats = F,height=5, width = 5*(length(list_of_plots))) #
print(ggarrange(plotlist = list_of_plots, ncol = length(list_of_plots), nrow = 1))
dev.off()
# i = 26 # HT270P1-S1H1A1US2_1 # G12D
# replace with G12V
i = 28 # HT060P1-S1R1Fp1U1 # G12V # we didn't end up using the plots for HT060P1 in the figures :`(
sample = input_table[i,"Sample_ID"]
rds_obj = input_table[i,"Xenium_snv_object"]
dir.create(paste0(out_dir,"/",sample))
Manual_cell_type = input_table[i,"Manual_xenium_cell_types_v6"]
...
obj@meta.data["KRAS_detected"][obj@meta.data[paste0(G12D_key,"_",assay,"_count")] > 0 & obj@meta.data[paste0(G12V_key,"_",assay,"_count")] ] <- "KRAS p.G12D p.G12V variant"
obj$KRAS_detected <- factor(obj$KRAS_detected, levels = c("KRAS p.G12 probe not detected","KRAS p.G12 reference","KRAS p.G12V variant","KRAS p.G12D variant","KRAS p.G12D p.G12V variant"))
zoom1.1 <- Crop(obj[["fov"]], x = c(11150, 11550), y = c(3890, 4290), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom1.1"]] <- zoom1.1
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom1'
#Simplified_cell_type = c("Normal" = "#009E73","Normal duct" = "#56B4E9","PanIN" = "#eee461","PDAC" = "#CC79A7")
DefaultBoundary(obj[["zoom1.1"]]) <- "segmentation"
...
obj$KRAS_detected <- factor(obj$KRAS_detected, levels = c("KRAS p.G12 probe not detected","KRAS p.G12 reference","KRAS p.G12V variant","KRAS p.G12D variant","KRAS p.G12D p.G12V variant"))
# use zoom1.1
zoom1.1 <- Crop(obj[["fov"]], x = c(1600, 2000), y = c(3750, 4150), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom1.1"]] <- zoom1.1
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom1'
#Simplified_cell_type = c("Normal" = "#009E73","Normal duct" = "#56B4E9","PanIN" = "#eee461","PDAC" = "#CC79A7")
DefaultBoundary(obj[["zoom1.1"]]) <- "segmentation"
...
obj$KRAS_detected <- factor(obj$KRAS_detected, levels = c("KRAS p.G12 probe not detected","KRAS p.G12 reference","KRAS p.G12V variant","KRAS p.G12D variant","KRAS p.G12D p.G12V variant"))
# use zoom1.1
zoom1.1 <- Crop(obj[["fov"]], x = c(4650, 5150), y = c(2350, 2850), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom1.1"]] <- zoom1.1
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom1'
#Simplified_cell_type = c("Normal" = "#009E73","Normal duct" = "#56B4E9","PanIN" = "#eee461","PDAC" = "#CC79A7")
DefaultBoundary(obj[["zoom1.1"]]) <- "segmentation"
...
obj$KRAS_detected <- factor(obj$KRAS_detected, levels = c("KRAS p.G12 probe not detected","KRAS p.G12 reference","KRAS p.G12V variant","KRAS p.G12D variant","KRAS p.G12D p.G12V variant"))
# use zoom1.1
zoom1.1 <- Crop(obj[["fov"]], x = c(5330, 5730), y = c(1550, 1950), coords = c("plot","tissue")) # the x and y coordinates here are the transposed coordinates from the region of interest targeted for the crop
obj[["zoom1.1"]] <- zoom1.1
#DefaultFOV(obj, assay='Xenium.with.snvs') <- 'zoom1'
#Simplified_cell_type = c("Normal" = "#009E73","Normal duct" = "#56B4E9","PanIN" = "#eee461","PDAC" = "#CC79A7")
DefaultBoundary(obj[["zoom1.1"]]) <- "segmentation"
...
# plotting the stacked bar plots showing proportion of KRAS p.G12D alleles in normal cells (Figure 6b and 6c)
library(epitools)
library(effectsize)
...
# Figure 6d DotPlot, hallmark enrichR pathways and associated module score VlnPlots and Extended data figure 9b
...
#plotting the merged umap Extended Data Figure 9a
library(Seurat)
library(tidyverse)
library(ggrastr)
library(ggpubr)
